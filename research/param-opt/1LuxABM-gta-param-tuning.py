...
def calculate_moments(data):
    ...
    return_dict['sigma2'] = sigma2
    return_dict['skew'] = skew
    return_dict['kurt'] = kurt
    return return_dict
...
dt = 1./252
for i, each in enumerate(SAMPLE_SPACE[27136: 27138]):
    ...
    data['param'] = {'tf': each[1], 'tc': each[2], 'nu': each[0], 'b': each[3]}
    if (calculate_moments(sim.vec_price)['average'] < 100) & (~np.isnan(calculate_moments(sim.vec_price)['average'])):
        trials.append(data)
        
        sys.stdout = open('candidates.txt','a+')
        print('Batch: ' + str(i))
        print('TS: ' + str(datetime.datetime.now()))
        print(data)
        print('##########')
    #print(data)
